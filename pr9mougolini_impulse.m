...
s_t = A*exp(1i*(2*pi*f_n.*t.^2./2*tau_imp)); %еденичный импульс %A*cos(2*pi*f_n.*t/2*tau_imp);
figure %еденичный импульс
plot(t,s_t);
xlim([0 2e-5])


s_t_n= [s_t zeros(1,round(fs*T_imp)-length(s_t))];%массив с импульсом и нулями 
